...
class AutoAttack():
    def __init__(self, model, norm='Linf', eps=.3, seed=None, verbose=True,
        ...
        #引发 ValueError（“attacks_to_run将被覆盖，除非您使用 version='custom'”）
        
        if not self.is_tf_model:  # 实例化攻击
        ...
        if version in ['standard', 'plus', 'rand']:
            self.set_version(version)   # 自定义版本
        
    def get_logits(self, x):  # 获取日志
        if not self.is_tf_model:
            return self.model(x)
        else:
            return self.model.predict(x)
    
    def get_seed(self):  #生成种子
        return time.time() if self.seed is None else self.seed
    
    def run_standard_evaluation(self,          # 运行标准评估
                                x_orig,
                                y_orig,
                                bs=250,
                                return_labels=False,
        ...
        if state_path is not None and state_path.exists():
            ...
            #创建状态时要运行一组不同的攻击。您可能使用了错误的状态文件。
            if self.verbose:
                self.logger.log("Restored state from {}".format(state_path))  #保存状态
                ...
                # 由于状态已恢复，因此只会返回当前运行的对抗性示例。
        else:
            state = EvaluationState(set(self.attacks_to_run), path=state_path)
            state.to_disk()
            if self.verbose and state_path is not None:
                self.logger.log("Created state in {}".format(state_path))           # 创建状态保存文件来记录日志                      

        attacks_to_run = list(filter(lambda attack: attack not in state.run_attacks, self.attacks_to_run))  #过滤不在列表里的攻击
        if self.verbose:
        ...
        # checks on type of defense 检查防御类型  
        if self.version != 'rand':  # 不是随机化防御
        ...
        n_cls = checks.check_range_output(self.get_logits, x_orig[:bs].to(self.device),
            logger=self.logger)   # 检查输出范围
        checks.check_dynamic(self.model, x_orig[:bs].to(self.device), self.is_tf_model,
            logger=self.logger)   # 检查动态防御
        checks.check_n_classes(n_cls, self.attacks_to_run, self.apgd_targeted.n_target_classes,
            self.fab.n_target_classes, logger=self.logger)         # 检查目标类
        
        with torch.no_grad():
            # calculate accuracy 计算准确率
            n_batches = int(np.ceil(x_orig.shape[0] / bs))  #分批
            if state.robust_flags is None:
                robust_flags = torch.zeros(x_orig.shape[0], dtype=torch.bool, device=x_orig.device)  #初始化全零张量
                ...
                for batch_idx in range(n_batches):
                    ...
                    output = self.get_logits(x).max(dim=1)[1]    # 预测值
                    y_adv[start_idx: end_idx] = output
                    correct_batch = y.eq(output)     #预测正确的数
                    robust_flags[start_idx:end_idx] = correct_batch.detach().to(robust_flags.device)

                state.robust_flags = robust_flags
                robust_accuracy = torch.sum(robust_flags).item() / x_orig.shape[0]
                robust_accuracy_dict = {'clean': robust_accuracy}
                state.clean_accuracy = robust_accuracy    # 干净数据的预测准确率
                
                if self.verbose:
                    self.logger.log('initial accuracy: {:.2%}'.format(robust_accuracy))  # 初始准确率
            else:  # robust_flags已经记录了，重新恢复状态后，只需计算准确率
            ...
            startt = time.time()
            for attack in attacks_to_run:  # 循环攻击
                # item() is super important as pytorch int division uses floor rounding 超级重要，因为 Pytorch int 部门使用floor舍入
                num_robust = torch.sum(robust_flags).item()  # 预测正确的数量
                print('num_robust',num_robust)
                ...
                robust_lin_idcs = torch.nonzero(robust_flags, as_tuple=False)   # 求出非零元素的索引（就是计划攻击预测正确的样本）
                if num_robust > 1:
                    robust_lin_idcs.squeeze_()
                
                for batch_idx in range(n_batches):   #按批
                    start_idx = batch_idx * bs
                    end_idx = min((batch_idx + 1) * bs, num_robust)

                    batch_datapoint_idcs = robust_lin_idcs[start_idx:end_idx]  # 每一批数据点的索引
                    if len(batch_datapoint_idcs.shape) > 1:
                        batch_datapoint_idcs.squeeze_(-1)  #降维
                    x = x_orig[batch_datapoint_idcs, :].clone().to(self.device)   #复制正确分类的样本
                    y = y_orig[batch_datapoint_idcs].clone().to(self.device)

                    # make sure that x is a 4d tensor even if there is only a single datapoint left
                    # 确保 X 是 4D 张量，即使只剩下一个数据点，  改成 2D（因为是流量数据，只有一维）
                    if len(x.shape) == 1:  # len(x.shape) == 3:
                        x.unsqueeze_(dim=0)
                    ...
                    if attack == 'apgd-ce':
                        ...
                        raise ValueError('Attack not supported')  # 不支持攻击
                
                    output = self.get_logits(adv_curr).max(dim=1)[1]
                    false_batch = ~y.eq(output).to(robust_flags.device)
                    non_robust_lin_idcs = batch_datapoint_idcs[false_batch]  # 预测错误的位置的索引
                    robust_flags[non_robust_lin_idcs] = False  # 修改状态->预测错误
                    state.robust_flags = robust_flags

                    x_adv[non_robust_lin_idcs] = adv_curr[false_batch].detach().to(x_adv.device)  #把对抗样本的预测错误的数据加载到x_adv(修改原始数据)

                    y_adv[non_robust_lin_idcs] = output[false_batch].detach().to(x_adv.device)  # 对抗样本的错误预测标签赋值给（y_adv)

                    if self.verbose:
                        num_non_robust_batch = torch.sum(false_batch)     # 预测错误的总数
                        self.logger.log('{} - {}/{} - {} out of {} successfully perturbed'.format(
                            attack, batch_idx + 1, n_batches, num_non_robust_batch, x.shape[0]))  # 每批中多少预测错了
                
                robust_accuracy = torch.sum(robust_flags).item() / x_orig.shape[0]
                robust_accuracy_dict[attack] = robust_accuracy  # 某个攻击后的预测准确率
                state.add_run_attack(attack)
                if self.verbose:
                    self.logger.log('robust accuracy after {}: {:.2%} (total time {:.1f} s)'.format(
                        attack.upper(), robust_accuracy, time.time() - startt))  #某个攻击后的准确率
                    
            # check about square  检查square攻击
        ...
        if return_labels:
            return x_adv, y_adv    # 返回对抗样本和生成的错误预测标签
        else:
            return x_adv
        
    def clean_accuracy(self, x_orig, y_orig, bs=250):    #计算数据的准确率
    ...
    def run_standard_evaluation_individual(self, x_orig, y_orig, bs=250, return_labels=False):  # 运行自定义版本的攻击
        if self.verbose:
    ...
    def set_version(self, version='standard'):  # 设置运行版本
        if self.verbose:
            print('setting parameters for {} version'.format(version))
        
        if version == 'standard':
            self.attacks_to_run = ['apgd-ce', 'apgd-t', 'fab-t']  #  , 'square'  #不使用这种攻击
            if self.norm in ['Linf', 'L2']:
                self.apgd.n_restarts = 1
                self.apgd_targeted.n_target_classes = 7
            ...
            self.fab.n_target_classes = 7
            #self.apgd_targeted.n_target_classes = 8
            self.square.n_queries = 5000
        
        elif version == 'plus':
            self.attacks_to_run = ['apgd-ce', 'apgd-dlr', 'fab', 'apgd-t', 'fab-t']  # , 'square'
            self.apgd.n_restarts = 5
