...
def check_range_output(model, x, alpha=1e-5, logger=None):
    ...
    if all(fl):
        msg = 'it seems that the output is a probability distribution,' +\
            ...
            f' See {checks_doc_path} for details.'  # 看来输出是概率分布，请确保使用了对数！
        if logger is None:
            warnings.warn(Warning(msg))
        else:
            logger.log(f'Warning: {msg}')
...
def check_square_sr(acc_dict, alpha=.002, logger=None):
    if 'square' in acc_dict.keys() and len(acc_dict) > 2:
        ...
        if acc_dict['square'] < acc - alpha:  # 方形攻击降低了
            msg = 'Square Attack has decreased the robust accuracy of' +\
                ...
                f' See {checks_doc_path} for details.'  #这可能表明使用“自动攻击”的鲁棒性评估是不可靠的。考虑运行Square使用更多迭代和重新启动或自适应攻击进行攻击。
            if logger is None:
                warnings.warn(Warning(msg))
            else:
                logger.log(f'Warning: {msg}')
...
def check_dynamic(model, x, is_tf_model=False, logger=None):
    if is_tf_model:
        msg = 'the check for dynamic defenses is not currently supported'  #当前不支持动态防御检查
    else:
        msg = None
        ...
        if any([c > 0 for c in funcs.values()]):
            msg = 'it seems to be a dynamic defense! The evaluation' + \
                ...
                f' See {checks_doc_path} for details.'  # 这似乎是一种动态防御！使用自动攻击进行评估可能不够。
    if not msg is None:
        if logger is None:
            warnings.warn(Warning(msg))
        else:
