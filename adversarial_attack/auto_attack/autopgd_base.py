...
class APGDAttack():
    ...
    def perturb(self, x, y=None, best_loss=False, x_init=None):
        """
        :param x:           clean images  干净图像
        :param y:           clean labels, if None we use the predicted labels  干净标签
        :param best_loss:   if True the points attaining highest loss    # True返回最好的损失, Flase返回对抗样本
        ...
        if not y is None and len(y.shape) == 0:
            x.unsqueeze_(0)
            y.unsqueeze_(0)
        self.init_hyperparam(x)  # 初始化超参数

        x = x.detach().clone().float().to(self.device)
        if not self.is_tf_model:
            y_pred = self.model(x).max(1)[1]
        else:
            y_pred = self.model.predict(x).max(1)[1]  # 预测结果
        if y is None:
            #y_pred = self.predict(x).max(1)[1]
            y = y_pred.detach().clone().long().to(self.device)
        else:
            y = y.detach().clone().long().to(self.device)  # y

        adv = x.clone()
        if self.loss != 'ce-targeted':
            acc = y_pred == y  # 无目标攻击，预测不等于真实（False），预测错的位置acc为0
        else:
            acc = y_pred != y  # 目标攻击，确实不等于，acc为True, 预测错的位置为1
        ...
        if not best_loss:
            ...
            for counter in range(self.n_restarts):
                ind_to_fool = acc.nonzero().squeeze()  # acc中1的个数 （默认无目标攻击，即预测对的位置）也就是计划攻击的位置
                if len(ind_to_fool.shape) == 0:
                    ind_to_fool = ind_to_fool.unsqueeze(0)
                if ind_to_fool.numel() != 0:
                    x_to_fool = x[ind_to_fool].clone()  # 将被扰动的样本
                    y_to_fool = y[ind_to_fool].clone()
                    
                    
                    if not self.use_largereps:
                        res_curr = self.attack_single_run(x_to_fool, y_to_fool)  # 运行攻击，返回攻击后的样本  （只扰动预测错误的样本）如果直接攻击原始数据，可能出现错变对的情况，成功率较低
                    else:
                        res_curr = self.decr_eps_pgd(x_to_fool, y_to_fool, epss, iters)
                    best_curr, acc_curr, loss_curr, adv_curr = res_curr
                    ind_curr = (acc_curr == 0).nonzero().squeeze()    # 扰动后预测错误的位置

                    acc[ind_to_fool[ind_curr]] = 0      # 扰动后预测错误的位置赋值0
                    adv[ind_to_fool[ind_curr]] = adv_curr[ind_curr].clone()  # 对抗样本赋值
                    if self.verbose:
                        print('restart {} - robust accuracy: {:.2%}'.format(   # 扰动后的预测准确率
            ...
            for counter in range(self.n_restarts):
                best_curr, _, loss_curr, _ = self.attack_single_run(x, y)  # 循环攻击
                ...
                if self.verbose:
                    print('restart {} - loss: {:.5f}'.format(
                        counter, loss_best.sum()))  #找到最好的损失

            return adv_best

    def decr_eps_pgd(self, x, y, epss, iters, use_rs=True):  #增大扰动后的攻击
...
class APGDAttack_targeted(APGDAttack):
    def __init__(
            ...
            rho=.75,
            topk=None,
            n_target_classes=7,  # 这里应该是7种，因为7种恶意流量
            verbose=False,
            device=None,
            use_largereps=False,
            is_tf_model=False,
            logger=None):
        """
        AutoPGD on the targeted DLR loss  针对目标 DLR 损耗的自动PGD
        """
    ...
    def dlr_loss_targeted(self, x, y):  # 有目标dlr损失
        x_sorted, ind_sorted = x.sort(dim=1)
        u = torch.arange(x.shape[0])
    ...
    def ce_loss_targeted(self, x, y):   # 有目标ce损失
    ...
    def perturb(self, x, y=None, x_init=None):
        ...
        #print(self.n_restarts) 1
        
        for target_class in range(2, self.n_target_classes + 2):   # 为啥从2开始.因为第二个标签是恶意流量，第一个是良性流量标签
            for counter in range(self.n_restarts):
                ...
                if ind_to_fool.numel() != 0:
                    ...
                    # # 最终就是减少和目标攻击类的损失
                    # print("原来的标签\n", y_to_fool[0:5])
                    # print('output.sort(dim=1)[1]\n',output.sort(dim=1)[1])
                    # print('target_class\n',target_class)
                    # print("要愚弄的目标标签\n",self.y_target[0:5])
                   
                    self.y_target = torch.zeros_like(y_to_fool) # 目标攻击类都设置为0
                    # print("原来的标签\n", y_to_fool[0:5])
                    # print("要愚弄的目标标签\n",self.y_target[0:5])
                    ...
                    #if self.verbose:  #详细  输出目标类的详细攻击信息结果
