...
class SquareAttack():
    ...
    def attack_single_run(self, x, y):
        with torch.no_grad():
            adv = x.clone()
            x = x.unsqueeze(0)  # 因为源代码处理图像数据是三维的，我的是一维流量数据，因此对其进行二次升维
            c, h, w= x.shape[1:]
            n_features = c * h * w  # 原来是三维的 c*h*w
            n_ex_total = x.shape[0]

            if self.verbose and h != w:
                print('square attack may not work properly for non-square image.')  #对于非方形图像，方形攻击可能无法正常工作
    ...
    def perturb(self, x, y=None):
        ...
        for counter in range(self.n_restarts):
            ind_to_fool = acc.nonzero().squeeze()  #  去掉这个试一下 .squeeze()
            ...
            if ind_to_fool.numel() != 0:
                ...
                _, adv_curr = self.attack_single_run(x_to_fool.unsqueeze(0), y_to_fool.unsqueeze(0))
