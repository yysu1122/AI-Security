...
class FABAttack():
    """
    Fast Adaptive Boundary Attack (Linf, L2, L1)  快速自适应边界攻击
    ...
    def attack_single_run(self, x, y=None, use_rand_start=False, is_targeted=False):
        ...
        :param is_targeted    True if we use targeted version. Targeted class is assigned by `self.target_class`  #有目标攻击
        """
        ...
        if self.verbose:
            print('Clean accuracy: {:.2%}'.format(pred.float().mean()))  # 预测的准确率
        if pred.sum() == 0:  # 全错则直接返回
            return x
        pred = self.check_shape(pred.nonzero().squeeze())

        if is_targeted:
            output = self._predict_fn(x)
            # la_target = output.sort(dim=-1)[1][:, -self.target_class]  #原来没有+1，是： -self.target_class
            # la_target2 = la_target[pred].detach().clone()

            la_target = torch.zeros_like(y)   # # 目标攻击类都设置为0
            la_target2 = la_target[pred].detach().clone()
            # print('output.sort(dim=-1)[1]',output.sort(dim=-1)[1])
            # print('la_target',la_target[0:5])
            # print('la_target2',la_target2[0:5])

        startt = time.time()
        # runs the attack only on correctly classified points  仅在正确分类的点上运行攻击
