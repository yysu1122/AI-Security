...
# # 将AutoAttack(无目标、有目标)+MLP, AutoAttack(无目标、有目标)+DNN, AutoAttack(无目标、有目标)+LSTM新对抗样本一同添加至原始训练数据集，并训练MLP/DNN/LSTM对抗模型，测试效率
...
# ### 基于采样训练集生成的加强训练集的Autoattack对抗样本

# In[ ]:


# Autoattack + 无目标 + MLP
under_train_adv_ex_auto_no_target_MLP= dataset.CICIDSDataset(
            features_file=f"{DATA_DIR_ADV_EXAMPLES}/adv_ex_final/under_train_adv_ex_auto_no_target_MLP.pkl",
            target_file=under_train_labels_path,
            transform=torch.tensor,
            target_transform=torch.tensor
            )

test_loader_under_train_adv_ex_auto_no_target_MLP = torch.utils.data.DataLoader(dataset=under_train_adv_ex_auto_no_target_MLP, batch_size=batch_size, shuffle=False)

# Autoattack + 无目标 + DNN
under_train_adv_ex_auto_no_target_DNN= dataset.CICIDSDataset(
            features_file=f"{DATA_DIR_ADV_EXAMPLES}/adv_ex_final/under_train_adv_ex_auto_no_target_DNN.pkl",
            target_file=under_train_labels_path,
            transform=torch.tensor,
            target_transform=torch.tensor
            )

test_loader_under_train_adv_ex_auto_no_target_DNN = torch.utils.data.DataLoader(dataset=under_train_adv_ex_auto_no_target_DNN, batch_size=batch_size, shuffle=False)

# Autoattack + 无目标 + LSTM
under_train_adv_ex_auto_no_target_LSTM= dataset.CICIDSDataset(
            features_file=f"{DATA_DIR_ADV_EXAMPLES}/adv_ex_final/under_train_adv_ex_auto_no_target_LSTM.pkl",
            target_file=under_train_labels_path,
            transform=torch.tensor,
            target_transform=torch.tensor
            )

test_loader_under_train_adv_ex_auto_no_target_LSTM = torch.utils.data.DataLoader(dataset=under_train_adv_ex_auto_no_target_LSTM, batch_size=batch_size, shuffle=False)

# Autoattack + 有目标 + MLP
under_train_adv_ex_auto_target_MLP= dataset.CICIDSDataset(
            features_file=f"{DATA_DIR_ADV_EXAMPLES}/adv_ex_final/under_train_adv_ex_auto_target_MLP.pkl",
            target_file=under_train_labels_path,
            transform=torch.tensor,
            target_transform=torch.tensor
            )

test_loader_under_train_adv_ex_auto_target_MLP = torch.utils.data.DataLoader(dataset=under_train_adv_ex_auto_target_MLP, batch_size=batch_size, shuffle=False)

# Autoattack + 有目标 + DNN
under_train_adv_ex_auto_target_DNN= dataset.CICIDSDataset(
            features_file=f"{DATA_DIR_ADV_EXAMPLES}/adv_ex_final/under_train_adv_ex_auto_target_DNN.pkl",
            target_file=under_train_labels_path,
            transform=torch.tensor,
            target_transform=torch.tensor
            )

test_loader_under_train_adv_ex_auto_target_DNN = torch.utils.data.DataLoader(dataset=under_train_adv_ex_auto_target_DNN, batch_size=batch_size, shuffle=False)

# Autoattack + 有目标 + LSTM
under_train_adv_ex_auto_target_LSTM= dataset.CICIDSDataset(
            features_file=f"{DATA_DIR_ADV_EXAMPLES}/adv_ex_final/under_train_adv_ex_auto_target_LSTM.pkl",
            target_file=under_train_labels_path,
            transform=torch.tensor,
            target_transform=torch.tensor
            )

test_loader_under_train_adv_ex_auto_target_LSTM = torch.utils.data.DataLoader(dataset=under_train_adv_ex_auto_target_LSTM, batch_size=batch_size, shuffle=False)
...
# 拼接数据集，将under_train_adv_ex_auto_no_target_MLP, under_train_adv_ex_auto_no_target_DNN, under_train_adv_ex_auto_no_target_LSTM, under_train_adv_ex_auto_target_MLP, under_train_adv_ex_auto_target_DNN, under_train_adv_ex_auto_target_LSTM新对抗样本一同添加至原始训练数据集
data_list = [train_data, under_train_adv_ex_auto_no_target_MLP, under_train_adv_ex_auto_no_target_DNN, under_train_adv_ex_auto_no_target_LSTM, under_train_adv_ex_auto_target_MLP, under_train_adv_ex_auto_target_DNN, under_train_adv_ex_auto_target_LSTM]
...
# 保存模型
# 新的MLP/DNN/LSTM模型
path = '../adversarial_train/adv_ex_train_models/AutoAttack_MLP_adv_model.pt'
# path = '../adversarial_train/adv_ex_train_models/AutoAttack_DNN_adv_model.pt'
# path = '../adversarial_train/adv_ex_train_models/AutoAttack_LSTM_adv_model.pt'
...
model2.to(device)
model2.load_state_dict(torch.load('../adversarial_train/adv_ex_train_models/AutoAttack_MLP_adv_model.pt'))
# model2 = DNN()
# model2.to(device)
# model2.load_state_dict(torch.load('../adversarial_train/adv_ex_train_models/AutoAttack_DNN_adv_model.pt'))

# model2 = LSTM(49,64,8,3)
# model2.to(device)
# model2.load_state_dict(torch.load('../adversarial_train/adv_ex_train_models/AutoAttack_LSTM_adv_model.pt'))
...
# ### 所以出现了上述防御结果：基于Autoattack的对抗训练模型也能一定程度上防御SINIFGSM攻击
